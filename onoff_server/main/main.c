...
int led_state_real = 0; // Assuming 0 for LED off, 1 for LED on

// Function to set the LED state
void set_led_state(int state)
{
    led_state_real = state;
    // Optionally, control your LED hardware here based on 'state'
}

// Function to get the LED state
int get_led_state(void)
{
    return led_state_real;
}

#define MAX_USER_CODE_LEN 20
static uint8_t user_code[MAX_USER_CODE_LEN];

void save_user_code(uint8_t *value, uint16_t len)
{
    if (len > MAX_USER_CODE_LEN)
    {
        ESP_LOGE(GATTS_TABLE_TAG, "User code length exceeds maximum allowed length");
        return;
    }
    memcpy(user_code, value, len);
    ESP_LOGI(GATTS_TABLE_TAG, "User code saved successfully");
}
...
#define ESP_GATT_UUID_LED_CONTROL 0xABF6
static const uint16_t spp_led_control_uuid = ESP_GATT_UUID_LED_CONTROL;
static const uint8_t spp_led_control_val[1] = {0x00};

// Initialize values

#define GATTS_CHAR_UUID_SPP_USER_CODE 0xABF7
static const uint16_t spp_user_code_uuid = GATTS_CHAR_UUID_SPP_USER_CODE;
uint8_t spp_user_code_val[20] = {0};
...
static const esp_gatts_attr_db_t spp_gatt_db[SPP_IDX_NB] =
    {
        ...
        [SPP_IDX_SPP_STATUS_VAL] =
            {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&spp_status_uuid, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE, SPP_STATUS_MAX_LEN, sizeof(spp_status_val), (uint8_t *)spp_status_val}},
        ...
        [SPP_IDX_SPP_LED_CHAR] =
            {
                {ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&character_declaration_uuid, ESP_GATT_PERM_READ, CHAR_DECLARATION_SIZE, CHAR_DECLARATION_SIZE, (uint8_t *)&char_prop_read_write}},

        [SPP_IDX_SPP_LED_VAL] =
            {
                {ESP_GATT_RSP_BY_APP}, {ESP_UUID_LEN_16, (uint8_t *)&spp_led_control_uuid, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE, 1, sizeof(spp_led_control_val), (uint8_t *)spp_led_control_val}},

        [SPP_IDX_SPP_USER_CODE_CHAR] =
            {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&character_declaration_uuid, ESP_GATT_PERM_READ, CHAR_DECLARATION_SIZE, CHAR_DECLARATION_SIZE, (uint8_t *)&char_prop_read_write}},

        // SPP - User code characteristic Value
        [SPP_IDX_SPP_USER_CODE_VAL] =
            {{ESP_GATT_RSP_BY_APP}, {ESP_UUID_LEN_16, (uint8_t *)&spp_user_code_uuid, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE, sizeof(spp_user_code_val), sizeof(spp_user_code_val), (uint8_t *)spp_user_code_val}},
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_READ_EVT:
        ...
        else if (res == SPP_IDX_SPP_LED_VAL)
        {
            ...
            rsp.attr_value.len = sizeof(led_state);    // Length of the response value
            rsp.attr_value.value[0] = get_led_state(); // Assign the LED state value
            printf("here i am for now");
            esp_err_t response_err = esp_ble_gatts_send_response(gatts_if, p_data->read.conn_id, p_data->read.trans_id,
                                                                 ESP_GATT_OK, &rsp);
            if (response_err != ESP_OK)
            {
                ESP_LOGE(GATTS_TABLE_TAG, "Sending response failed: %s", esp_err_to_name(response_err));
            }
        }
        else
        {
    ...
    case ESP_GATTS_WRITE_EVT:
    {
        ...
        if (p_data->write.is_prep == false)
        {
            ...
            else if (res == SPP_IDX_SPP_LED_VAL)
            {
                ...
                {
                    // Assuming the value of the LED control characteristic determines LED state
                    led_state_real = p_data->write.value[0];
                    // Example: Set LED state based on received value (0 = OFF, 1 = ON)
                    if (led_state_real == 0x01)
                    {
                        // Turn LED ON
                        ...
                        rsp.attr_value.value[0] = 0; // You can set an appropriate response value if needed
                ...
                else if (res == SPP_IDX_SPP_USER_CODE_VAL)
                {
                    uint8_t *value = p_data->write.value;
                    uint16_t len = p_data->write.len;
                      ESP_LOGI(GATTS_TABLE_TAG, "SPP_IDX_SPP_USER_CODE_VAL");
                    save_user_code(value, len);
                }
            }
